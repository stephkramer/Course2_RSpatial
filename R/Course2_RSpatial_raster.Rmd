...
In R, these are _Spatial*DataFrame_ (the * can stand for *Points*, *Lines* or
...
```{r package-install}
pkgs = c("raster", "stars", "rgdal", "rgeos", "rasterVis", "sf",  
         "ggplot2", "tmap", "viridis", "patchwork", "here", "units")
# install.packages(pkgs) #lease run this line on your device
# update.packages()
```
...
## do NOT load both packages terra and raster, as this creates problems with the namespace 
library(raster) ## will be outdated soon and updated with {terra}
library(rgdal)
library(rgeos)
library(rasterVis)
...
library(sf)
library(stars)
library(sp) ## will be outdated soon and updated with {terra}
...
## the old way
#dbor_wd     <- paste0(root_wd, "/", "_DataBorneo") 
...
ras_bio_asc_01@crs    ## crs = coordinate reference system. If empty, assign it!
```
<br>
...
`@crs` gives the information of the object (here: raster layer) `Bor_mat` or `ras_bio_asc_01`, and
...
`CRS("+proj=longlat +datum=WGS84")` use `CRS("+init=epsg:4326")`.
...
ras_bio_asc_24@crs <- ras_bio_asc_01@crs
...
```{r, fig.height=5}
extent(x = ras_bio_asc_01) ## same as extent@ras_bio_asc_01
...
bbox(obj = ras_bio_asc_01_cr) ## similar to:
ras_bio_asc_01_cr@extent      ## equal to:
extent(ras_bio_asc_01_cr)     ## equal to:
attr(ras_bio_asc_01_cr,'extent')

## analoguously:

## xmin (lower left corner)
ras_bio_asc_01_cr@extent@xmin
# xmin(ras_bio_asc_01_cr) # xmin(extent(ras_bio_asc_01_cr))

## cells/ columns in x-direction
ras_bio_asc_01_cr@ncols        
# ncol(ras_bio_asc_01_cr) ## !n.b. cols vs. col!

## accessing all values from upper left to lower right as one vector:
## note: I use head() to shorten the vector length
head(ras_bio_asc_01_cr@data@values)  # head(values(ras_bio_asc_01_cr))

## coordinate reference system: the location on the globe
ras_bio_asc_01_cr@crs
# crs(ras_bio_asc_01_cr) 
# projection(ras_bio_asc_01_cr) # != CRS(ras_bio_asc_01_cr) !!!!!

# RECAP
## crs = coordinate reference system, when already defined
## if not defined, CRS creates/ sets projection and takes args for crs as e.g. 
# proj4string(ras_bio_asc_01_cr) <- CRS("+proj=longlat +datum=WGS84") OR
# ras_bio_asc_01@crs <- CRS(projargs = "+proj=longlat +datum=WGS84")
```

```{r ras-data-structure-3}
## Retrieve internal data cont.:
nrow(x = ras_bio_asc_01_cr) # ncol(ras_bio_asc_01_cr) # ncell(ras_bio_asc_01_cr)
dim(x = ras_bio_asc_01_cr)  # 12 rows, 30 columns, 1 z-dimension
res(x = ras_bio_asc_01_cr)  # resolution = cell size
```
<br>

An important function is *coordinates()*, which returns the centroids of
each raster cell:

```{r centroid}
head(x = coordinates(obj = ras_bio_asc_01_cr), n = 10) ## command from package 'raster'
```
<br>
...
(m <- tm_shape(ras_bio_asc_01) +
  tm_raster(palette = "magma",  title = "Mean annual temp"))
...
converted to a conventional data frame first, i.e. 2 steps are needed prior to
plotting. The object `SpatialPointsDataFrame` is explained in more detail when
working with vector data.

```{r}
## First, convert RasterLayer object to a SpatialPointsDataFrame
ras_bio_asc_01_pts <- rasterToPoints(ras_bio_asc_01)
...
# ggplot(data = data.frame(rasterToPoints(ras_bio_asc_01))) + ...
```
...
Now we illustrate how to plot objects created with the old `{sp}` package.
